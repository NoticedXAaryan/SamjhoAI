...
const CVBackground = memo(() => {
  ...
  useEffect(() => {
    ...
    let activeConstellations: Constellation[] = [];
    ...
    const render = () => {
      ...
      if (activeConstellations.length < 3 && Math.random() < 0.02) { // More frequent, up to 3 at once
        ...
        if (availableIndices.length > 0) {
          ...
          if (validPosition) {
            const selectedParticles: Particle[] = [];
      ...
      for (let i = activeConstellations.length - 1; i >= 0; i--) {
        ...
        if (constellation.timer >= constellation.maxTime) {
          constellation.particles.forEach((p: Particle) => {
            p.inConstellation = false;
            p.targetX = null;
            p.targetY = null;
          });
              ...
              >
                ...
                <motion.div style={{ scale: screenScale, y: screenY }} className="w-full h-full relative origin-center">
                  ...
                  <div className="w-full h-full relative font-sans bg-[#0a0a0a]">
                    ...
                    <Image src="https://images.unsplash.com/photo-1573496359142-b8d87734a5a2?q=80&w=800&auto=format&fit=crop" alt="Main speaker" fill sizes="700px" className="object-cover opacity-90" referrerPolicy="no-referrer" unoptimized />
                    ...
                    <div className="absolute top-10 sm:top-4 right-2 sm:right-4 w-20 sm:w-32 aspect-video bg-gray-800 rounded-lg overflow-hidden border border-white/20 shadow-lg">
                      <Image src="https://images.unsplash.com/photo-1580489944761-15a19d654956?q=80&w=400&auto=format&fit=crop" alt="Local user" fill sizes="128px" className="object-cover" referrerPolicy="no-referrer" unoptimized />
                    </div>
